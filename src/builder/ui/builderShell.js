...
function renderNewMapForm(appState) {
  ...
  return `
    ...
    <section class="builder-landing-context">
      <div class="builder-inspector-note">Pencil rule: this creates only the simple base map. Export is available for the blank map; terrain brushes, structures, and spawns are separate later passes.</div>
    </section>
  `;
}
...
function renderInspector({ builderState, refs, appState }) {
  if (!isBuilderWorkspaceMap(builderState)) {
    ...
    refs.inspector.innerHTML = `
      ...
      <div class="builder-inspector-note">
        New Blank Map creates builder-owned map data. Export downloads map JSON plus a mapList entry helper.
      </div>
    `;
    return;
  }
  ...
  const note = builderState.workspaceMode === "builder-map"
    ? "This map is builder-owned. Export downloads map JSON plus a mapList entry helper. Terrain/structure mutation tools are still locked."

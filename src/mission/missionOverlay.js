...
function renderCampaignRewardSummary(state) {
  ...
  if (reward) {
    if (reward.applied) {
      ...
      rows.push(...renderProgressionRows(reward.progression));
      if (reward.items?.length) rows.push(`<div><strong>Items:</strong> ${escapeHtml(reward.items.join(", "))}</div>`);
      if (reward.recruitedPilots?.length) rows.push(`<div><strong>Recruited:</strong> ${escapeHtml(reward.recruitedPilots.join(", "))}</div>`);
...
function renderProgressionRows(progression) {
  if (!progression || Number(progression.milestoneLevels ?? 0) <= 0) return [];

  const rows = [`<div><strong>Level Milestone:</strong> +${escapeHtml(progression.milestoneLevels)}</div>`];
  const active = Array.isArray(progression.activeLevelUps) ? progression.activeLevelUps : [];
  const reserves = Array.isArray(progression.reserveCatchUps) ? progression.reserveCatchUps : [];

  if (active.length) {
    rows.push(`<div><strong>Active Level Ups:</strong> ${escapeHtml(formatLevelChanges(active))}</div>`);
  }

  if (reserves.length) {
    rows.push(`<div><strong>Reserve Catch-up:</strong> ${escapeHtml(formatLevelChanges(reserves))}</div>`);
  }

  rows.push(`<div><strong>Reserve Floor:</strong> ${escapeHtml(progression.reserveFloor ?? 1)}</div>`);
  return rows;
}

function formatLevelChanges(changes) {
  return changes
    .map((change) => `${change.pilotId} ${change.from}->${change.to}`)
    .join(", ");
}
